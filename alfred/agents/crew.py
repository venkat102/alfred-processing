...
TASK_DESCRIPTIONS = {
	...
	"generate_changeset": {
		"description": (
			...
			"- For any field/perm-touching item, call `get_doctype_context(<target>)` once to\n"
			"  confirm fieldnames + source tags before writing, and `get_doctype_perms(<target>)`\n"
			"  for role/permlevel tasks. Use `find_field(<target>, <hint>)` when a fieldname\n"
			"  might be a typo or partial name.\n"
			"- BEFORE Final Answer, call `validate_changeset` on your assembled array. Fix any\n"
			"  `critical` issues and re-validate (one retry only). Do not emit a Final Answer\n"
			"  with known critical issues - the savepoint dry-run will reject them anyway.\n"
