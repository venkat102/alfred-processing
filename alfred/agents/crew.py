...
LITE_TASK_DESCRIPTION = (
	...
	"the task has FAILED.\n\n"
	"THINK FIRST, ACT SECOND (reasoning discipline - not in Final Answer):\n"
	"Your very first Thought MUST begin with THREE lines in this exact order,\n"
	"before any tool call:\n"
	"  Target DocType: <the EXACT DocType name the user named in their request>\n"
	"  What user wants: <one sentence restating the user's goal>\n"
	"  PLAN:\n"
	"  1. create <doctype> '<name>' - <why, 1 line>\n"
	"  2. create <doctype> '<name>' - <why, 1 line>\n"
	"  (maximum 6 items, no sub-bullets)\n"
	"\n"
	"Every `<doctype>` in the PLAN must match your Target DocType line OR be a\n"
	"Frappe meta-doctype (Notification, Server Script, Client Script, Custom\n"
	"Field, Workflow, Report). It MUST NOT be a different domain doctype just\n"
	"because a tool docstring or template used one as an example.\n"
	"\n"
	"Only after writing Target + What + PLAN do you start calling tools. The\n"
	"plan stays in Thought: Final Answer is raw JSON only, no plan text.\n\n"
	...
	"1. Write `Target DocType:` + `What user wants:` + PLAN in your first Thought.\n"
	"2. Call lookup_doctype on the Target DocType from step 1 (NOT on any other\n"
	"   doctype that appeared in a tool docstring example). Use the result\n"
	"   internally - do NOT narrate it in your Final Answer.\n"
	...
	"   - **Validation / business rule / custom constraint** (user says: validate,\n"
	"     restrict, reject, throw, block, prevent, require, only allow, cannot\n"
	"     unless, must be) -> **Server Script** on the existing target DocType.\n"
	"     This is the MOST COMMON request shape and the one most often misrouted.\n"
	"     A validation rule is NEVER a Notification.\n"
	"     A validation rule is NEVER a new DocType.\n"
	"     If the user says 'throw a message' they mean frappe.throw, not an email.\n"
	"     Concrete shape for this case:\n"
	"       op='create', doctype='Server Script', data:\n"
	"         script_type='DocType Event'\n"
	"         reference_doctype=<Target DocType from step 1>\n"
	"         doctype_event='before_save' or 'before_insert' or 'validate'\n"
	"         script body: uses `doc.<field>` and calls `frappe.throw('<user-facing message>')`\n"
	"           to block the save when the rule fails.\n"
	"     If the validation needs a field that doesn't exist on the target DocType\n"
	"     yet (e.g. age_at_join), create a Custom Field in the SAME changeset -\n"
	"     still NO new DocType.\n"
	"   - Email / alert when a document changes -> Notification DocType\n"
	"   - New field on an existing DocType -> Custom Field\n"
	"   - Multi-state approval with transitions -> Workflow\n"
	"   - Brand new entity the user wants to track -> Only THEN create a new DocType\n"
	...
	"- Do NOT write documentation-style prose about what a DocType contains.\n"
	"- Do NOT echo back the output of lookup_doctype as your Final Answer.\n"
	"- Do NOT switch the target DocType between Thought and Final Answer.\n"
	"- Do NOT output example 'how to create a <doctype>' snippets - the Final\n"
	"  Answer is the changeset itself, not a tutorial."
)
