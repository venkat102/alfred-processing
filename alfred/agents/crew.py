...
INSIGHTS_TASK_DESCRIPTION = (
	...
	"   get_list, validate_name_available. Prefer `lookup_doctype` with\n"
	"   `layer=\"site\"` or `layer=\"both\"` over `get_doctype_schema`.\n"
	...
	"4. get_list cannot do: aggregations (SUM / AVG / GROUP BY beyond COUNT),\n"
	"   joins across doctypes, derived computations, or arbitrary SQL. If the\n"
	"   user asks for any of those, DO NOT fabricate an answer from schema.\n"
	"   Instead, reply with exactly this shape:\n"
	"     'I can list raw records from one doctype but I can't [aggregate /\n"
	"     join / compute]. You can either:\n"
	"     - rephrase as a simpler list or count (e.g. \"list customers\",\n"
	"       \"count open orders\"), or\n"
	"     - switch to Dev mode and I'll help you build a Report for this.'\n"
